const streamingRouter = require('./src/Routers/streaming/streaming.js');
const videoStreamingRouter = require('./src/Routers/streaming/videoStreamingRouter.js');
...
const connection = require('./src/utils/DB_connection.js');
const passport = require('passport');
const userRouter = require('./src/Routers/user.router.js');
const movieRouter = require('./src/Routers/movie.router.js');
dotenv.config();
var count = 0;
...
const ThumbnailDestnitation = multer({
...
app.use('/protected-route/moviedetails', movieRouter);
...
app.get('/checksession', (req, res) => {
	...
	console.log('sessioncheck-endpoint :', req.user);
...
app.post('/adminlogin', bodyParser.urlencoded({ extended: true }), adminAuthenticate, (req, res) => {
	...
	res.redirect(`http://${process.env.SERVER}:${process.env.PORT}/protected-route/views/console/html/VideoGallery.html`);
});
...
app.post(
	...
	(req, res) => {
		...
		// console.log(req);
		connection.query(SqlQuery, [MovieID, req.files.video[0].originalname, req.body.description, genre, req.body.language, req.body.Olanguage, req.body.rating, cname, dname, req.body.rdate], (err, result, fields) => {
			if (!err) {
